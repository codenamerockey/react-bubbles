...
- [ ] Explain what a token is used for.
  ...
  1.  The reason why JWT's are used is to encrypt data that is stored on a server, and is a security measure. to protect private data and only allow that data to be viewed after proper authentication from the source requesting that data has been verified.

- [ ] What steps can you take in your web apps to keep your data secure?

  2.  You can create private routes, so only users that have been authenticated can see the data.

- [ ] Describe how web servers work.

  3. A Web server is a program that uses HTTP (Hypertext Transfer Protocol) to serve the files that form Web pages to users, in response to their requests.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
  4. Post is Create, Get is Read, Update is U, and Delete is D
